...
建造者设计模式的核心思想是将一个复杂对象的构建过程分解为多个简单的步骤，每个步骤由一个具体的建造者类负责实现。
这些步骤按照一定的顺序被调用，最终构建出一个完整的对象。
结构
...
public:
  ...
  void constructPizza()
  {
    m_builder->builderDough();
    m_builder->builderSauce();
    m_builder->builderTopping();
  }
...
int main()
{
  ...
  HawwaiianPizzaBuilder hawaiianBuilder;
