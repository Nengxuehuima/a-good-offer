核心思想
抽象工厂设计模式是一种创建型设计模式，它提供了一种创建一系列相关或相互依赖对象的接口，而无需指定其具体类。通过定义一个抽象工厂接口，声明一组用于创建不同产品族的抽象方法，每个具体的工厂类实现这个接口，并负责创建特定的产品族。具体产品类实现了产品接口，每个工厂类创建的产品都属于同一个产品族，但是产品族之间有不同的实现。
...
示例
开发一个图形绘制应用程序，需要根据用户的选择创建不同类型的图形对象，如按钮（Button）和文本框（Textbox）。可以使用抽象工厂模式来实现这个场景。
...
public:
  void render() override{
    cout<<"Rendering a Mac textbox"<<std::endl;
  }
};
...
int main()
{
  ...
  //创建一个Mac风格的工厂
  GUIFactory* macfactory = new MacFaactory();
  //创建一个Mac风格的按钮
