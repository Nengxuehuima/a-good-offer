...
示例
假设我们有一个图形库，其中有一个基类 Shape 表示图形，它有一个纯虚函数 draw() 用于绘制图形。
现在我们希望能够复制已有的图形对象来创建新的图形对象，而无需重新构造图形对象。
...
private:
  ...
  int radius;
public:
  Circle(std::string color,int redius) : color(color),radius(radius) {}
...
int main()
{
  ...
  Shape* circle1 = circleProtorype->clone();
  Shape* circle2 = circleProtorype->clone();
