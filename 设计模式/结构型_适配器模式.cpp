核心思想
适配器设计模式（Adapter Design Pattern）是一种结构型设计模式，用于将一个类的接口转换为另一个类的接口，以满足客户端的需求。
适配器模式允许不兼容的类能够合作，通过适配器将一个类的接口转换为另一个类的接口，使得两者可以无缝协同工作。
主要组成部分
...
//类适配器
class Adapter : public Target,private Adatee{
public:
