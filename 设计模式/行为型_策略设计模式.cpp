策略设计模式（Strategy Design Pattern）是一种行为型设计模式，它定义了一系列算法，并将每个算法封装到具体的策略类中，使得它们可以互相替换。
通过使用策略模式，可以在运行时动态地选择算法，而不需要修改调用算法的代码。
核心思想
...
private:
  Strategy* strategy;
public:
  void setStrategy(Strategy* strategy)//setStrategy() 是一个普通成员函数，可以在对象创建之后随时更换策略。允许 动态切换策略
  {
